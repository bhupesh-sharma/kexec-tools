 ...
 *
 * Created by: Bhupesh Sharma (bhsharma@redhat.com)
...
uint64_t phys_offset = UINT64_MAX;
uint64_t kimage_voffset  = UINT64_MAX;
...
unsigned machine_pointer_bits(void)
{
	uint8_t bits = 0;
...
void scan_vmcoreinfo(char *start, size_t size)
{
	...
	char *str, *endp;
	long phys_offset_l = NOT_FOUND_LONG_VALUE;
...
#define SYMBOL(sym) {					\
	...
	for (pos = start; pos <= last; pos = eol + 1) {
		...
		*eol = '\0';
		...
		if (memcmp(str, pos, strlen(str)) == 0) {
			phys_offset_l = strtoul(pos + strlen(str), &endp,
							10);
			if (strlen(endp) != 0)
				phys_offset_l = strtoul(pos + strlen(str), &endp, 16);
			if ((phys_offset == LONG_MAX) || strlen(endp) != 0)
				fprintf(stderr, "Invalid data %s\n",
					pos);
			else
				fprintf(stderr, "phys_offset=%lx\n",
					phys_offset_l);
		}
		
		str = "KERNELOFFSET=";
		if (memcmp(str, pos, strlen(str)) == 0) {
			kaslr_offset = strtoul(pos + strlen(str), &endp,
							16);
			fprintf(stderr, "kaslr_offset=%lx\n",
					kaslr_offset);
		}
		...
		if (memcmp(str, pos, strlen(str)) == 0) {
			...
			if ((phys_offset == LONG_MAX) || strlen(endp) != 0)
				...
				fprintf(stderr, "kimage_voffset=%lx\n",
					kimage_voffset);
		}
...
int scan_notes(int fd, loff_t start, loff_t lsize)
{
...
uint64_t read_file_pointer(int fd, uint64_t addr)
{
	uint64_t result;
	ssize_t ret;
...
uint32_t read_file_u32(int fd, uint64_t addr)
{
	uint32_t scratch;
	ssize_t ret;
	ret = pread(fd, &scratch, sizeof(scratch), addr);
...
int32_t read_file_s32(int fd, uint64_t addr)
{
	return read_file_u32(fd, addr);
}

void write_to_stdout(char *buf, unsigned int nr)
{
	ssize_t ret;
...
void dump_dmesg_structured(int fd)
{
...
int read_elf_vmcore(int fd)
{
	...
	ret = read_elf(fd);
	if (ret > 0) {
		fprintf(stderr, "Unable to read ELF information from vmcore\n");
		return ret;
	}
...
int read_elf_kcore(int fd)
{
	...
	ret = read_elf(fd);
	if (ret < 0) {
		fprintf(stderr, "Unable to find VMCOREINFO PT_NOTE in '/proc/kcore'\n");
		return ret;
	}
	
	if (ret > 0) {
		fprintf(stderr, "Unable to read ELF information from '/proc/kcore'\n");
		return ret;
	}
...
int read_phys_offset_elf_kcore(int fd, unsigned long *phys_off)
{
	...
	if (!ret) {
		/* If we have a valid 'PHYS_OFFSET' by now, return it to the
		 * caller now
		 */
